...
class DbUpgradeV6 implements DbUpgrade {
  ...
  // Rebuilds supply_items so that medication-only columns are nullable and
  // adds the polymorphic `type` discriminator + `amount` column for generic
  // supplies. Foreign key enforcement is disabled by AppDatabase during
  // migrations: dropping the old table would otherwise trigger ON DELETE
  // SET NULL on medication_intakes.supplyItemId.
